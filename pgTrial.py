...
#samrtsheet trial
# Initialize Smartsheet client with your API access token
smartsheet_client = smartsheet.Smartsheet('v2JfboaYuoy1IMnWR1Fh2XXYXodJXikVOlpel')

# Read the Smartsheet by its sheet ID
...
#     with open("./templates/template-profile.txt", "r") as template_file:
...
print(f"Writing to: {subfolder_path.resolve()}")
...
#ends here
for i, row in df.iterrows():   # ✅ this defines 'i'
    file_name = safe_names[i] + ".pcf"   # use the cleaned name we generated earlier
